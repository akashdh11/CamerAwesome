...
- (NSData *)writeMetadataIntoImageData:(NSData *)imageData
  ...
  // add the image contained in the image source to the destination, overidding
  // the old metadata with our modified metadata
...
#pragma clang diagnostic pop
  ...
  selfReference = nil;
  if (error) {
    _completion(nil, [FlutterError errorWithCode:@"CAPTURE ERROR"
...
#pragma clang diagnostic pop
  ...
  UIImage *imageConverted =
      [self imageByCroppingImage:image
                          toSize:CGSizeMake(outputWidth, outputHeight)];

  image = [UIImage imageWithCGImage:[imageConverted CGImage]
                              scale:0.0
                        orientation:[self getJpegOrientation]];

  NSData *imageWithExif =
      [UIImageJPEGRepresentation(image, 1.0) addExif:container];

  bool success = [imageWithExif writeToFile:_path atomically:YES];
  if (!success) {
    _completion(nil, [FlutterError errorWithCode:@"IOError"
...
- (UIImageOrientation)imageOrientationFromDeviceOrientation:
    (UIDeviceOrientation)devOrient {
  switch (devOrient) {
  case UIDeviceOrientationPortrait:
    return (self.sensorPosition == PigeonSensorPositionFront &&
            _mirrorFrontCamera)
...
- (UIImageOrientation)imageOrientationFromInterfaceOrientation:
